...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  HomeBloc(
    ...
    on<ChangePageEvent>((event, emit) =>
        emit(state.rebuild((b) => b..selectedPage = event.pageNumber)));

    ///FriendshipRequests
    on<GetFriendshipRequestsEvent>((event, emit) async {
      emit(state.rebuild((b) => b
        ..isLoading = true
        ..isSuccess = false
        ..error = ''));
      final result =
          await friendshipRequestsRemoteDataSource.friendshipRequests();
    ...
    on<AddStatusEvent>((event, emit) async {
      emit(state.rebuild((b) => b..error = ''));
      final result =
          await addStatusRemoteDataSource.addStatus(status: event.file);
      return result.fold((l) async {
        print('l');
    ...
    ///كل الاصدقاء
    on<GetAllFriendEvent>((event, emit) async {
      emit(state.rebuild((b) => b
        ..error = ''
        ..isLoadingGetAllFriend = true
        ..isSuccessGetAllFriend = false));
      final result = await allFriendRemoteDataSource.allFriend();
      return result.fold((l) async {
        print('l');
        emit(state.rebuild((b) => b..error = l));
        emit(state.rebuild((b) => b
    ...
    on<ChangeRequestsFriend>((event, emit) async {
      emit(state.rebuild((b) => b..error = ''));
      final result = await changeRequestsFriendRemoteDataSource
          .changeRequestsFriend(id: event.id, status: event.status);
      return result.fold((l) async {
        print('l');
        emit(state.rebuild((b) => b..error = l));
        emit(state.rebuild((b) => b..error = ''));
    ...
    ///بحث عن صديق
    on<GetSearchFriendEvent>((event, emit) async {
      emit(state.rebuild((b) => b
        ..error = ''
        ..isLoadingSearch = true
        ..isSuccessSearch = false));
      final result =
          await searchFriendRemoteDataSource.searchFriend(search: event.search);
      return result.fold((l) async {
        print('l');
        emit(state.rebuild((b) => b
    ...
    ///seenStatus
    on<SeenStatusEvent>((event, emit) async {
      emit(state.rebuild((b) => b..error = ''));
      final result =
          await seenStatusRemoteDataSource.seenStatus(statusId: event.statusId);
      return result.fold((l) async {
        print('l');
        emit(state.rebuild((b) => b..error = l));
        emit(state.rebuild((b) => b..error = ''));
    ...
    on<GetStatusEvent>((event, emit) async {
      emit(state.rebuild((b) => b
        ...
        ..isLoading = true
        ..isSuccess = false));
      final result = await getStatusRemoteDataSource.getStatus();
  ...
  void onChangeRequestsFriend({required int id, required String status}) {
    add(ChangeRequestsFriend(status: status, id: id));
  }

  void onAddStatusEvent({required File status}) {
    add(AddStatusEvent(file: status));
  }

  void onSeenStatusEvent(
      {required int statusId, required int index, required int seenNumber}) {
    add(SeenStatusEvent(
        statusId: statusId, index: index, seenNumber: seenNumber));
  }
