...
class _ConversationScreenState extends State<ConversationScreen> {
  ...
  initWithContext(BuildContext context) {
    if (first) {
      ...
      if (args!.user != null) {
        UserData user = UserData.fromJson(json.decode(args!.user!));
      ...
      if (args!.conversationId == "-1") {
        ...
        bloc.onGetConversationMessage(args!.conversationId, page);
        conIdNotFound = false;
      }
    ...
    return BlocConsumer<ConversationBloc, ConversationState>(
      ...
      listener: (context, state) {
        ...
        if (state.sendMessageModel.data!.conversation_id! > -1) {
          if (conIdNotFound) {
            channelChat = Global.pusher!.subscribe(
                "chat.privet.${state.sendMessageModel.data!.conversation_id}");
        ...
        if (state.getConversationIdModel.data!.isNotEmpty) {
          if (state.getConversationIdModel.data![0].messages!.isNotEmpty) {
            bloc.onGetConversationMessage(
                state.getConversationIdModel.data![0].messages![0]
                    .conversation_id!,
                page);
          }
        }
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                         args!.online =='1'?'online'.tr(): args!.lastSeen ?? '',
                                          style: TextStyle(
                                              color: ColorManager
                                                  .lightGreyShade200,
                          ...
                          children: [
                            ...
                            Expanded(
                              child: ListView.separated(
                                ...
                                itemCount:
                                    state.privateOldMessageModel.data!.length,
                                itemBuilder: (context, index) {
                                  if (Global.userId ==
                                      state.privateOldMessageModel.data![index]
                                          .user!.id) {
                                    if (checkIsVideo(
                                        state.privateOldMessageModel
                                            ...
                                            : null,
                                        state.privateOldMessageModel
                                            .data![index].localFile)) {
                                      return SizedBox(
                                        height: 170.h,
                                        child: MessageVideoSideOne(
                                          message: state.privateOldMessageModel
                                              .data![index],
                                        ),
                                      );
                                    ...
                                    return MessageChatSideOne(
                                      message: state
                                          .privateOldMessageModel.data![index],
                                    );
